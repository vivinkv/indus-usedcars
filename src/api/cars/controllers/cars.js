...
module.exports = {
  getCars: async (ctx, next) => {
    try {
      const fetchCars = await axios.post('http://tvapp.indusmis.in/ServiceTV.svc/getUsedCarDetails', {
        outlet: '',
        pageNumber: "1",
        pageSize: "2000"
      }, {
      ...
      for (const carData of fetchCars.data.getUsedCarDetailsResult) {
        ...
        const checkVehicleRegistration = await strapi.documents('api::car.car').findFirst({
          filters: {
        ...
        if (!checkVehicleRegistration) {
          ...
          if (!brand) {
            brand = await strapi.documents('api::brand.brand').create({
              data: {
                Name: carData.Make,
                Slug: carData?.Make?.replaceAll(' ','-')?.replaceAll(/[^a-zA-Z0-9]/g, '')?.toLowerCase()
              }
            });
          ...
          if (!model) {
            model = await strapi.documents('api::model.model').create({
              data: {
                Name: carData?.Model || String.fromCharCode(...Array.from({ length: 10 }, () => Math.floor(Math.random() * 26) + 97)),
                Slug: carData?.Model?.replaceAll(' ','-')?.replaceAll(/[^a-zA-Z0-9]/g, '')?.toLowerCase()
              }
            });
          ...
          if (!fuel) {
            fuel = await strapi.documents('api::fuel-type.fuel-type').create({
              data: {
                Name: carData?.Fuel_Type,
                Slug: carData?.Fuel_Type?.replaceAll(' ','-')?.replaceAll(/[^a-zA-Z0-9]/g, '')?.toLowerCase()
              }
            });
          ...
          if (!outlet) {
            outlet = await strapi.documents('api::outlet.outlet').create({
              data: {
                Name: carData?.Outlet,
                Slug: carData?.Outlet?.replaceAll(' ','-')?.replaceAll(/[^a-zA-Z0-9]/g, '')?.toLowerCase()
              }
            });
            await strapi.documents('api::outlet.outlet').publish({ documentId: outlet.documentId });
          }

          let vehicle_category = await strapi.documents('api::vehicle-category.vehicle-category').findFirst({
            filters: {
          ...
          if (!vehicle_category && carData?.Vehicle_Category) {
            vehicle_category = await strapi.documents('api::vehicle-category.vehicle-category').create({
              data: {
                Name: carData.Vehicle_Category,
                Slug: carData.Vehicle_Category?.replaceAll(' ','-')?.replaceAll(/[^a-zA-Z0-9]/g, '')?.toLowerCase()
              }
            });
          ...
          const data = await strapi.documents('api::car.car').create({
            data: {
              brand: brand,
              model: model,
              outlet: outlet,
            ...
            populate: ['brand', 'fuel_type', 'model', 'vehicle_category', 'outlet']
          });
